...
function PricingContent({ initialUserState }: { initialUserState?: ServerUserState }) {
  ...
  // "Probar Pro 1€" CTA to users who have already used their trial.
  // Once the hooks settle, trust them (covers cross-tab purchases).
  const authLoading = !isLoaded || campusLoading;
  const useServer = authLoading && !!initialUserState;
  const hasPro = useServer ? initialUserState!.hasPro : hookHasPro;
  const isTrialing = useServer ? initialUserState!.isTrialing : hookIsTrialing;
  const hasUsedTrial = useServer ? initialUserState!.hasUsedTrial : hookHasUsedTrial;
  const effectiveIsSignedIn = useServer ? initialUserState!.isSignedIn : !!isSignedIn;

  const userHasPro = effectiveIsSignedIn && hasPro;
...
export default function PricingClient({
  ...
  return (
    ...
    >
      <PricingContent initialUserState={initialUserState} />
    </Suspense>
  );
}
