...
export default function LessonLoading() {
  ...
  return (
    <div className="flex h-full overflow-hidden animate-pulse bg-mx-bg" aria-busy="true">
        ...
        <div className="mt-2 flex flex-col gap-2">
          {rows.map((_, i) => (
            <div key={i} className="flex items-center gap-3 py-1.5">
              <div className="h-5 w-5 shrink-0 rounded-full bg-black/[0.08]" />
              <div
                className="h-3 rounded bg-black/[0.08]"
      ...
      {/* Contenido */}
      <div className="flex-1 min-w-0 overflow-hidden bg-mx-bg">
        {/* Barra superior (breadcrumb) */}
        <div className="sticky top-0 z-30 bg-mx-bg/85 backdrop-blur-sm border-b border-mx-border px-6 py-3 flex items-center gap-3">
          <div className="h-5 w-5 rounded bg-black/[0.08]" />
          <div className="min-w-0 flex-1">
            <div className="h-2 w-32 rounded bg-black/[0.08]" />
            <div className="mt-2 h-3 w-64 max-w-[60%] rounded bg-black/[0.08]" />
          </div>
        ...
        <div className="px-6 md:px-12 lg:px-16 py-10 max-w-4xl mx-auto">
          <div className="h-9 w-3/4 rounded-lg bg-black/[0.08]" />
          <div className="mt-8 flex flex-col gap-3">
            {lines.map((_, i) => (
              <div
                key={i}
                className="h-3 rounded bg-black/[0.07]"
                style={{ width: `${88 - ((i * 9) % 30)}%` }}
              />
            ))}
          </div>
          {/* Placeholder de imagen */}
          <div className="mt-10 h-72 w-full rounded-xl border border-mx-border bg-black/[0.04]" />
          <div className="mt-8 flex flex-col gap-3">
