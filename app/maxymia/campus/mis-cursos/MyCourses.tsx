  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-8 h-8 border-2 border-mx-border border-t-mx-blue rounded-full animate-spin" />
      </div>
    );
  }
    ...
    return (
      <div className="w-full py-10">
        <m.div
          ...
          className="rounded-xl border border-mx-border bg-mx-card p-10 text-center max-w-md mx-auto"
        >
          <div className="w-16 h-16 rounded-2xl bg-black/[0.04] border border-mx-border flex items-center justify-center mx-auto mb-6">
            <BookOpen size={28} className="text-mx-text-muted" />
          </div>
          <h1 className="text-heading-sm md:text-heading-md font-bold text-mx-text mb-3">
            {t('myCourses.emptyTitle')}
          </h1>
          <p className="text-mx-text-muted text-body-sm md:text-body-md mb-8">
            {t('myCourses.emptyDesc')}
          </p>
          <Link
            href="/maxymia/campus/cursos"
            className="inline-flex items-center gap-2 px-6 py-3 bg-mx-orange hover:bg-mx-orange-dark text-white font-semibold rounded-lg transition-colors text-body-sm"
          >
      ...
      <section className="mb-8">
        <div className="w-full">
          ...
          >
            ...
            <p className="text-mx-text-muted text-body-sm mt-1 max-w-lg">
              {stats.inProgress > 0
          ...
          {stats.globalPercent > 0 && (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-mx-text-muted text-label-md">{t('myCourses.globalProgress')}</span>
      ...
      <div className="w-full pb-8">
        ...
        {notStartedCourses.length > 0 && (
          <CourseRow
            ...
            icon={<Clock size={18} className="text-mx-text-muted" />}
        ...
        {completedCourses.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-5 px-1">
              ...
              <h2 className="text-body-lg md:text-heading-sm 2xl:text-heading-md font-semibold text-mx-text">
                {t('myCourses.completedSection')}
              </h2>
              <span className="text-label-sm text-mx-text-muted ml-1">({completedCourses.length})</span>
            </div>
...
function StatCard({
  ...
  const styles = {
    default: {
      border: 'border-mx-border',
      iconBg: 'bg-black/[0.04]',
      icon: 'text-mx-text-muted',
      value: 'text-mx-text',
    },
    blue: {
  ...
  return (
    <div
      onClick={handleClick}
      className={`relative overflow-hidden rounded-xl bg-mx-card border ${s.border} p-5 ${scrollTo ? 'cursor-pointer hover:shadow-sm transition-shadow' : ''}`}
    >
      <div className={`w-10 h-10 rounded-xl ${s.iconBg} flex items-center justify-center mb-4`}>
        <div className={s.icon}>{icon}</div>
      </div>
      <p className={`text-heading-md font-black ${s.value}`}>{value}</p>
      <p className="text-label-sm uppercase tracking-widest text-mx-text-muted mt-1">{label}</p>
    </div>
  );
}
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2">
          {icon}
          <h2 className="text-mx-text text-body-lg md:text-heading-sm 2xl:text-heading-md font-semibold">{title}</h2>
          <span className="text-label-sm text-mx-text-muted ml-1">({courses.length})</span>
        </div>
        {courses.length > 3 && (
